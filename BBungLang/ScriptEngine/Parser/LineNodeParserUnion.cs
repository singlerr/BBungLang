...
public static class LineNodeParserUnion
{
    ...
    public static readonly Func<List<Node>, List<Node>> InfixToPostfix = delegate(List<Node> list)
    {
        var stack = new Stack<Node>();
        var postfix = new List<Node>();
        foreach (var node in list)
            if (node.NodeType == NodeType.StartRoundBracket)
            {
                stack.Push(node);
            }
            else if (node.NodeType.IsOperator())
            {
                if (stack.Count == 0)
                {
                    stack.Push(node);
                ...
                while (stack.Count != 0 && stack.Peek().NodeType.GetInStackPriority() <=
                       node.NodeType.GetInComingPriority())
                    postfix.Add(stack.Pop());


                stack.Push(node);
            }
            else if (node.NodeType == NodeType.EndRoundBracket)
            {
                Node top;
                while (stack.Count != 0 && (top = stack.Pop()).NodeType != NodeType.StartRoundBracket) postfix.Add(top);
            }
            else
            {
                postfix.Add(node);
    ...
    public static Func<List<Node>, Node> CalcPostfix = delegate(List<Node> list)
    {
        var stack = new Stack<Node>();
        foreach (var node in list)
            if (!node.NodeType.IsOperator())
            {
                stack.Push(node);
            }
            else
            {
                if (stack.Count < 2)
                    throw new Exception($"Operator requires two operands but found {stack.Count}");
                var rightOperand = stack.Pop();
                var leftOperand = stack.Pop();
                var result = ExecOperator(node.NodeType, leftOperand, rightOperand);
                stack.Push(result);
            }
    ...
    public static readonly Func<List<Node>, Node> CalcConstants = delegate(List<Node> nodes)
    {
        ...
        */
        return CalcPostfix(InfixToPostfix(nodes));
    };
    ...
    public static Func<LineNode, Collection<object>> GoTo = delegate(LineNode node)
    {
        var args = new Collection<object>();

        if (node.GetHeadNodeType() != NodeType.GoTo)
            throw new Exception($"Called {NodeType.GoTo} but found {node.GetHeadNodeType()}");

        var internalNodes = node.InternalNodes;

        if (internalNodes.Count != 2)
            throw new Exception($"Expected 1 arguments but found {internalNodes.Count - 1}");

        var destNode = NodeUtils.GetRawOrVariableValue(internalNodes[1]);

        if (destNode.NodeType != NodeType.String)
            throw new Exception($"Expected string but found {destNode.NodeType}");

        args.Add(destNode.Value);
