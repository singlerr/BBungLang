    ...
    public class ScriptLexer
    {
        ...
        public Collection<Node> LexInternalNodes(string line)
        {
            ...
            for (var charIdx = 0; charIdx < line.Length; charIdx++)
            {
                ...
                if (NodeType.StartRoundBracket.GetExpression()[0] == currentChar)
                {
                    var roundBracketNode = new ElementNode(NodeType.StartRoundBracket, currentValue);
                    internalNodes.Add(roundBracketNode);
                    buffer.Clear();
                }

                if (NodeType.EndRoundBracket.GetExpression()[0] == currentChar)
                {
                    var roundBracketNode = new ElementNode(NodeType.EndRoundBracket, currentValue);
                    internalNodes.Add(roundBracketNode);
                    buffer.Clear();
                }
                ...
                if (NodeType.GoTo.GetExpression().Equals(currentValue))
                {
                    var goToNode = new ElementNode(NodeType.GoTo, currentValue);
                    internalNodes.Add(goToNode);
                    buffer.Clear();
                }
            }
