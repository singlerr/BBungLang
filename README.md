﻿# About BBungLang
이 스크립트 엔진은 모 게임에서 작성된 스크립트를 이해하기 위한 인터프리터입니다.
이 스크립트 문법을 조금 수정하여 BBungLang을 만들었습니다.
...
스크립트는 반드시 "aa{" 으로 시작하고, } 로 끝나야 합니다.
...
주의! 반드시 숫자와 연산자, 그리고 (와 )는 서로 띄워져 있어야 합니다.
...
예를 들어 a 라는 변수에 1 이라는 값, ab_1 이라는 변수에 "바보"라는 값을 넣는 구문은 다음과 같습니다.
```
빵 { "a" : 1 , "ab" : "바보" }
```
...
예를 들어 "1. 변수 할당"에서 할당한 "a" 변수 값을 가져오려면 $a 로 표기하면 됩니다.
...
> 이 명령어는 게임에서 캐릭터의 상태를 설정하던 것이었습니다. 
>> 즉, 게임 스크립트로 사용되지 않는 지금, 변수 설정 명령어와 다를게 없습니다.
> 따라서 아래의 설명에는 게임에서 사용되는 기능이 포함되어 있습니다.
...
### 5. Segment Set 이동
> 이 명령어는 게임에서 캐릭터의 상태를 설정하던 것이었습니다. 명령어가 작동하기는 하나, 큰 의미는 없습니다.
...
### 아래부터는 게임에 사용하기 위해 만든 기능입니다. BBungLang에서는 사용하지 않습니다.
### 7. 선택지 생성

선택지를 만드는 과정은 선택지 화면 생성 > 선택지 추가 > 선택지 선택 후 실행될 구문 등록 순으로 이어집니다.
